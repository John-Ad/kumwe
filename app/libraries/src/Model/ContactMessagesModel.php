 ...
 * Model class
 */
class ContactMessagesModel implements DatabaseModelInterface
{
    ...
    public function setItem(
        ...
        if ($id > 0) {
            ...
            try {
                $db->updateObject('#__contactMessages', $data, 'id');
            ...
            try {
                $db->insertObject('#__contactMessages', $data);
            } catch (\RuntimeException $exception) {
                throw new \RuntimeException($exception->getMessage(), 404);
            }
    ...
    public function getItem(?int $id): \stdClass
    {
        ...
        $query = $db->getQuery(true)
            ->select('*')
            ->from($db->quoteName('#__contactMessages'))
    ...
    public function delete(int $id): bool
    {
        ...
        // Purge the session
        $query = $db->getQuery(true)
            ->delete($db->quoteName('#__contactMessages'))
            ->where($db->quoteName('id') . ' = :id')
            ->bind(':id', $id, ParameterType::INTEGER);
        try {
